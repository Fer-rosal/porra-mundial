...
export async function PUT(
  ...
  try {
    ...
    if (allPhasesError) {
      return internalErrorResponse(allPhasesError, 'PUT /api/games/:id/phase/:phase_key/open')
    }

    return successResponse({ tournament_phases: allPhases || [] })
  } catch (error) {
    
    return internalErrorResponse(error, 'PUT /api/games/:id/phase/:phase_key/open')
  }
}
